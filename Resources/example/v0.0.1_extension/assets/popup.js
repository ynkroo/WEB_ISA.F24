...
function updatePopupLanguage(language) {
  ...
  let footerContent = document.getElementsByTagName('footer')[0].getElementsByTagName('p');
  if (language == "en") {
    notOnPage.getElementsByTagName('p')[0].textContent = "To see your current ECTS points";
    ...
    footerContent[1].textContent = "Please note the number of ECTS points may be incorrect. You have no guarantee for these points. No data is saved or analysed by this extension."
  } else {
    notOnPage.getElementsByTagName('p')[0].textContent = "Um deine aktuellen ECTS-Punkte zu sehen";
    ...
    footerContent[1].textContent = "Die angezeigten ECTS-Punkte können inkorrekt sein, es besteht keine Garantie. Diese Erweiterung speichert oder analysiert keine Daten."
  }
}

// Function to handle language change
function handleLanguageChange() {
  let selectedLanguage = document.querySelector('input[name="languageswitch"]:checked').value;
...
chrome.storage.sync.get('language', function(data) {
  ...
  let savedRadio = document.querySelector(`input[name="languageswitch"][value="${selectedLanguage}"]`);
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  if (message.sendECTS) {
    ...
    let onPage = document.getElementById('onPage');
    let ectsDetails = document.getElementById("ectsDetails").getElementsByClassName("ectsNum");
    
    onPage.getElementsByClassName('ectsNum')[0].textContent = ects.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
